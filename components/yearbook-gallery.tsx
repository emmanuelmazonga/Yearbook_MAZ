    ...
    <div>
      ...
      <div className="grid auto-rows-[220px] gap-3 sm:grid-cols-3">
        {visible.map((photo) => (
          <button key={photo.id} onClick={() => setActive(photo)} className={"focus-ring group relative overflow-hidden bg-black text-left " + photo.span}>
            <img src={photo.image} alt={photo.alt} className="h-full w-full object-cover opacity-85 transition duration-700 group-hover:scale-105 group-hover:opacity-100" style={{ objectPosition: photo.position }} />
      ...
      <Dialog open={!!active} onOpenChange={(open) => !open && setActive(null)}>
        <DialogContent className="max-w-6xl border-0 bg-[#101c1d] p-3 text-white">
          ...
          {active && <><img src={active.image} alt={active.alt} className="max-h-[78vh] w-full object-contain" /><div className="px-3 pb-3"><p className="eyebrow text-[#f1ca7e]">{active.category}</p><p className="display mt-2 text-3xl">{active.title}</p><p className="mt-2">{active.caption}</p></div></>}
        </DialogContent>
      </Dialog>
    </div>
  );

 ...
 const docs=await content(); const settings=docs.find(d=>d._id==="siteSettings"); const book=docs.find(d=>d._type==="yearbook" && d._id===settings?.featuredYearbook?._ref);
  return (
    <footer className="bg-[#132f31] text-[#fffaf0]">
      <div className="page-shell grid gap-12 py-16 md:grid-cols-[1.4fr_.6fr_.6fr]">
        ...
        <div>
          ...
          <div className="grid gap-3 text-sm">
            <Link href="/schools">Schools</Link><Link href={book ? "/yearbooks/"+book.slug.current : "/schools"}>Yearbooks</Link><Link href="/photography">Photography</Link><Link href="/for-schools">For Schools</Link>
          </div>
        </div>
        <div>
      ...
      <div className="border-t border-white/10 py-5 text-center text-xs text-white/45">{settings?.footerText}</div>
    </footer>
  );
}

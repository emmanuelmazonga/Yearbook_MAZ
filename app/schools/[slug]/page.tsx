...
export async function generateMetadata({params}: {params: Promise<{slug:string}>}) {
  ...
  return pageMetadata({title:school.name,description:school.description || `Explore ${school.name}, its graduating classes and digital yearbook archive.`,path:`/schools/${slug}`,image:imageUrl(school.coverImage,1200) || null});
}
...
export default async function SchoolPage({params}: {params: Promise<{slug:string}>}) {
  ...
  return <main className="bg-[var(--school-secondary)] text-[var(--school-secondary-ink)]">
    <section className="relative min-h-[78svh] overflow-hidden bg-[var(--school-primary)] text-[var(--school-primary-ink)]">
      {school.coverImage && <img src={imageUrl(school.coverImage,1800)} alt={school.coverImage.alt || `${school.name} school community`} fetchPriority="high" decoding="async" className="absolute inset-0 h-full w-full object-cover opacity-70"/>}
      ...
      <div className="page-shell relative z-10 flex min-h-[78svh] items-end pb-14 pt-28"><div className="max-w-4xl">
        <div className="mb-8 flex items-center gap-5">
          ...
          <div>{school.establishedYear && <p className="eyebrow text-[var(--school-accent-on-primary)]">Established {school.establishedYear}</p>}<p className="mt-2 flex items-center gap-2 text-sm opacity-75"><MapPin className="size-4"/> {school.city}, {school.country}</p></div>
        </div>
        <h1 className="display text-6xl leading-[.9] tracking-[-.05em] sm:text-8xl">{school.name}</h1>
        {school.motto && <p className="mt-7 text-xl italic text-[var(--school-accent-on-primary)]">{school.motto}</p>}
    ...
    <section className="section-space" style={{background:withAlpha(theme.primary,.09)}}><div className="page-shell"><p className="eyebrow text-[var(--school-primary-on-secondary)]">Yearbook archive</p><h2 className="display mt-4 text-5xl sm:text-7xl">Choose a graduating class.</h2>

    ...
    <main className="section-space bg-[#132f31]">
      <div className="page-shell grid overflow-hidden lg:grid-cols-[.8fr_1.2fr]">
        <div className="flex flex-col justify-between bg-[#701d33] p-8 text-white sm:p-12">
          ...
          <div className="mt-10 border-t border-white/20 pt-6 text-sm leading-7 text-white/62"><p className="font-semibold tracking-wide text-[#f1ca7e]">Emmanuel Mazonga · Creative Lead</p><p className="mt-4">Photography · Digital yearbooks · Print editions</p><p>Kitwe and the wider Copperbelt, Zambia</p>{settings?.contactEmail && <p><a href={"mailto:"+settings.contactEmail}>{settings.contactEmail}</a></p>}{settings?.contactPhone && <p>{settings.contactPhone}</p>}</div>
        </div>
        <ContactForm />
      </div>
    </main>
